...
contract UniswapV3PoolTest is Test {
    ...
    function testMintSuccess() public {
        ...
        (bool lowerTickInitialized, uint128 lowerTickLiquidity) = pool.ticks(
            params.lowerTick
        );
        assertTrue(lowerTickInitialized);
        assertEq(lowerTickLiquidity, params.liquidity);
        //check if lowerTick gets initialized and equals pool liquidity
        (bool upperTickInitialized, uint128 upperTickLiquidity) = pool.ticks(
            params.upperTick
        );
    ...
    function testMintInvalidLowerTickRange() public {
        ...
        pool = new UniswapV3Pool(address(token0), address(token1), 0, 0);
    ...
    function testSwapBuyEth() public {
        ...
        assertEq(
            ...
            "invalid ETH balance in pool"
        );
        assertEq(
            token1.balanceOf(address(pool)),
            uint256(int256(poolBalance1) + amount1Delta),
            "invalid USDC balance in pool"
        );
